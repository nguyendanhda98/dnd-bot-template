...
export const loadCommands = async (client: ExtendedClient) => {
  ...
  for (const folder of commandFolders) {
    const commandFiles = fs
      .readdirSync(path.join(__dirname, "../commands/slash", folder))
      .filter((file) => file.endsWith(".ts") || file.endsWith(".js"));
...
export const loadInteractions = async (client: ExtendedClient) => {
  ...
  for (const file of buttonFiles) {
    const buttonModule = require(path.join(
      __dirname,
    ...
    if (button && button.id && typeof button.execute === "function") {
      ...
      logger.warn(
        `Button at ${file} is missing required "id" or "execute" property`
      );
    }
  }
  ...
  for (const file of selectMenuFiles) {
    const menuModule = require(path.join(
      __dirname,
    ...
    if (
      ...
      logger.warn(
        `Select menu at ${file} is missing required "id" or "execute" property`
      );
    }
  }
  ...
  for (const file of modalFiles) {
    const modalModule = require(path.join(
      __dirname,
      "../interactions/modals",
      file
    ));
    ...
    if (modal && modal.id && typeof modal.execute === "function") {
      ...
      logger.warn(
        `Modal at ${file} is missing required "id" or "execute" property`
      );
    }
  }
};
